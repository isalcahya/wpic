...
namespace App\lib;
/**
 * Plugin API: wcic_load_hook class
 *
 * @package WordPress
 * @subpackage Plugin
 * @since 4.7.0
 ...
 * @see ArrayAccess
 */
final class wcic_load_hook implements \Iterator, \ArrayAccess {
	...
	/**
	 * Normalizes filters set up before WordPress has initialized to wcic_load_hook objects.
	 *
	 * @since 4.7.0
	 *
	 * @param array $filters Filters to normalize.
	 * @return wcic_load_hook[] Array of normalized filters.
	 */
	public static function build_preinitialized_hooks( $filters ) {
		/** @var wcic_load_hook[] $normalized */
		$normalized = array();

		foreach ( $filters as $tag => $callback_groups ) {
			if ( is_object( $callback_groups ) && $callback_groups instanceof wcic_load_hook ) {
				$normalized[ $tag ] = $callback_groups;
				continue;
			}
			$hook = new wcic_load_hook();
